...
def main():
    ...
    parser.add_argument('--output-dir', '-o', required=True, help='output directory with the performance '
                                                                  'metrics files')
    parser.add_argument('--x-axis', choices=['proportion', 'norm'], required=True,
                        help="Choice of variable on the x-axis. Options are 'norm' for the perturbation norm, "
                             "and 'proportion' for proportion of adversarial/OOD sampless.")
    ...
    parser.add_argument('--hide-legend', action='store_true', default=False,
                        help='use this option to hide legends from the plots')
    ...
    if results:
        plot_performance_comparison(results, plot_dir, args.x_axis, place_legend_outside=plo,
                                    hide_legend=args.hide_legend, pos_label=args.pos_label, log_scale=args.log_scale,
                                    hide_errorbar=True, name_prefix=args.name_prefix)
    else:
